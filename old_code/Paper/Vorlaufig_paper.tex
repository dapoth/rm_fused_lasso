...
Following, we assume that $p$ is much larger then $N$, further written as $p>>N$. In that case $X$ does not have full rank and therefore the least squares estimate does not yield a unique solution. In order to construct an estimator, the 'sparsity' assumption can be imposed. Sparsity means that most of the elements in $\beta$ are equal to $0$, and will therefore be irrelevant for our model . Let $\lambda > 0$, then the estimator is defined as follows.
...
\noindent In matrix notation we can write this more compactly as
...
\noindent where $ \| \cdot \|_2^2 $ is the squared Euclidean norm. Note that this is equivalent to a constrained minimization problem of the form 
...
For fixed $\lambda_1$ the lasso estimator can be calculated by the coordinate descent algorithm. We can use coordinate descent because the objective function has the structure which is necessary for the algorithm to converge to the global minimum. The structure is\begin{equation}
...
As mentioned above the Lasso estimator does not make use of existing structure within the features. We now want to look at the case where we assume a natural ordering of the coefficients for example that each coefficient corresponds to a position on a straight line and that coefficients are closely related to their neighbors. We consider the fused lasso estimator which takes into account the ordering of the features by including another penalty as follows. 

	\begin{equation}
		\hat{\beta}_{\lambda_1,\lambda_2}^{FLassoSA}=\argmin_{\beta \in\rm \mathbb{R}^p} \left\{ \frac{1}{2n}  \sum_{i=1}^{N}(y_{i}-x_{i}'\beta)^2+\lambda_1 \sum_{j=1}^p|\beta_j|+ \lambda_2 \sum_{j=2}^p|\beta_j- \beta_{j-1}| \right \}	
	\end{equation}

\subsubsection{Fused Lasso Signal Approximator}
...
A special case of the fused lasso occurs when the predictor matrix is the $N \times N$ identity matrix. In this case the estimator has the following form.
	...
	\begin{equation}
	\hat{\beta}_{\lambda_1,\lambda_2}^{FLassoSA}=\argmin_{\beta \in\rm \mathbb{R}^p} \left\{ \frac{1}{2n}  \sum_{i=1}^{N}(y_{i}-\beta_i)^2+\lambda_1 \sum_{j=1}^p|\beta_j|+ \lambda_2 \sum_{j=2}^p|\beta_j- \beta_{j-1}| \right \}	
...
One useful property of the fused lasso estimator (only) in this easy setting is that of monotone fusion.
...
As described above the first penalty $\lambda_1$ encourages a sparse solution. The additional penalty $\lambda_2$ encourages sparsity in the differences, i.e. flatness or smoothness of the coefficient profiles $\beta_j$ as a function of the index set $j$ .

\subsubsection{Properties of the solution}

Similarly as for the Lasso above we can define the following properties of the Fused Lasso estimator.
...
For simplicity the authors assumed that $p$ is fixed with $N \rightarrow \infty$. As the authors note themselves these are ``not particularly realistic asymptotic conditions'' as the lasso and fused lasso are methods for high-dimensional data, which is not the case any more if $N$ gets larger than $p$. Preferably $p \rightarrow \infty$ and this faster than $N$.
...
\begin{theorem}
	...
	\begin{equation*}
		\sqrt{N}(\hat{\beta_N} - \beta) \rightarrow_{\text{d}} \argmin(V),
	\end{equation*}
	where
	\begin{align*}
...
The idea of the theorem and proof is similar to that in \citep{asymptoticslasso} where the asymptotics of the Lasso are analyzed. In order to study the limiting behavior of the fused lasso the asymptotic behavior of the objective function is studied.
...
Meaning of $\lambda_N^{(l)}/\sqrt{N} \rightarrow \lambda_0^{(l)} \geq 0 \  (l=1,2)$: $\lambda$ is $o(n)$ \todo{Describe this further.}
...
In section \ref{Sec: Coordinate Lasso} we saw that the conditions for the coordinate descent to be a suitable solution method are fulfilled for the lasso. Unfortunately for the fused lasso this is not the case. The reason is that the separated non differentiable functions $h$ do not only depend on a single coefficient but on two. Take for example the fused signal approximator case, where we can set $\lambda_1 = 0$ (compare lemma \ref{sa: soft thresholding}). Assume we have $p=100$ and coordinate descent sets the coefficients successfully to their optimal value. \todo{Dann würde der Algorithmus doch terminieren?} Further, assume that $\beta_1=\beta_2$. Now let's say we are in a new iteration step and minimize over $\beta_1$. Observe any change in $\beta_1$ introduces at least an increase of the objective because of the absolute penalty. \todo{Das stimmt nur in manchen Szenarien.}Maybe it would be slightly optimal to increase $\beta_1$ a little to get rid of some squared differences in the model fit but since the error is small the absolute penalty would overwhelm this effect so it is optimal for $\beta_1$ to stay the same. Now if we optimize over $\beta_2$ we face the same problem as before. If we were in the same situation as for $\beta_1$ namely that increasing it a little would be better but it isn't done because of the combined penalty. Then coordinate descent is stuck. Therefore we have to use algorithms like gradient descent because they can move $\beta_1$ and $\beta_2$ together so in this case both increase slightly so the squared error gets smaller and the difference stays 0.
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\textwidth]{red_line}
	\caption{CGH Data}
	\label{cghdata}
\end{figure}
