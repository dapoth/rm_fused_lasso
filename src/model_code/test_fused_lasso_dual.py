...
def expected_beta():
    out_signal = {}
    out_signal['expected_beta'] = [0, 2.5]
    
    out_general = {}
    out_general['expected_beta'] = [1, 1]
    
    out = {"values_signal": out_signal, "values_general": out_general}
    return out

expected_beta_signal = [0, 2.5]
expected_beta_general = [1, 1]
    
def test_lasso_signal_dual(setup_param_signal):
    calculated_beta_dual = fused_lasso_dual(**setup_param_signal["data_dual"])
    assert_allclose(expected_beta_signal, calculated_beta_dual, atol=1e-2)
    
def test_lasso_signal_primal(setup_param_signal):
    calculated_beta_primal = fused_lasso_primal(**setup_param_signal["data_primal"])
    assert_allclose(expected_beta_signal, calculated_beta_primal, atol=1e-2)
    
def test_lasso_general_dual(setup_param):
    calculated_beta_dual = fused_lasso_dual(**setup_param["data_dual"])
    assert_allclose(expected_beta_general, calculated_beta_dual, atol=1e-2)
