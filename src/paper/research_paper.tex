...
\noindent where $R = \frac{||Y||^2}{\lambda}$, thus $R$ is dependent on the data \todo{konkreter} and on the penalty constant.\footnote{See the appendix for details.}
...
\frac{1}{n}\sum_{i=1}^n x_{ij}(y_i-x_i'b)= \frac{1}{2}\lambda sgn(b_{\lambda,j}) &\ if \quad b_{\lambda,j} \neq0 \\   
\frac{1}{n}\sum_{i=1}^n x_{ij}(y_i-x_i'b) \leq  \frac{1}{2}\lambda \ &\ if \quad b_{\lambda,j}=0
\end{dcases} 
\end{align*}
...
\end{equation}
where $g: \mathbb{R}^p \to \mathbb{R}$, differentiable and convex and $h_j(\beta_j)$ are convex. \todo{Brauchen eine Zitierung dieser Bedingungen} The squared Euclidean norm and the absolute value functions fulfill these criteria. An important observation is that the functions only depend on one $\beta$ coefficient.
