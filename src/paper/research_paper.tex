...
\begin{python}
def generate_data(n, n_features, num_simulations, number_blocks, length_blocks,
                  amplitude, spike_level, levels=False, spikes=0):
    """Generate the data for the simulation."""

    beta = np.zeros([n_features, num_simulations])
    for sim in range(num_simulations):
        beta[:, sim] = generate_beta(n_features, number_blocks, length_blocks,
...
\begin{python}
def generate_beta(n_features, number_blocks, length_blocks, amplitude,
                  levels=False, spikes=0, spike_level):
    """Generate betas with desired block structure for
    simulation purposes."""

    container = np.zeros(n_features)
    max_blocks = math.floor(n_features / length_blocks)
...
def fused_lasso_dual(y, x, lambda1, lambda2):
    ...
    obj = cp.Minimize(error + lambda1 * cp.norm(b, 1) +
                      lambda2 * cp.norm(b[1:n_features]-b[0:n_features-1], 1))
    prob = cp.Problem(obj)
    prob.solve()
...
def fused_lasso_primal(y,x,s1,s2):
    ...
    constraints = [cp.norm(b,1) <= s1, cp.norm(b[1:n_features]-b[0:n_features-1],1) <= s2]
