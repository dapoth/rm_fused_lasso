...
%\begin{figure}
%\includegraphics[width=\textwidth]{../../src/sandbox/figures/plot_solutionpath_lasso.png}
...
On table ?! we see that fused lasso and fusion perform well for
...
rarther be suited for lasso, the overall performance of fused lasso
...
\begin{figure}		\includegraphics[width=\textwidth]{../../src/sandbox/figures/plot_fused.pdf}
...
The idea of the proof is to show that the soft-thresholded estimator solves the gradient equations for the fused lasso problem with penalties $\lambda_1$ and $\lambda_2$. Since the gradient equations are necessary and sufficient for it to be a solution the proof is then complete. The subgradient equations for the fused lasso signal approximator are given by:
	...
	\textbf{Case 1: $|\hat{\beta}_{0,i}| > \lambda$} In this case $\hat{\beta}_{\lambda,i} > 0$. Plugging the estimator in the subgradient equation and reordering of terms gives
	\begin{equation}
	...
	\textbf{Case 2: $|\hat{\beta}_j^0|< \lambda_1$ }
...
Next we compare the differences $sgn(\hat{\beta}_j^0-\hat{\beta}_{j-1}^0)$ with $sgn(\hat{\beta}_j^{\lambda_1}-\hat{\beta}_{j-1}^{\lambda_1})$. There are two cases: if after the thresholding one parameter is set to 0 and the other one unequal to zero we know that betas just changes in the level so the differences stays the same, the other case is if both get shrunken to zero, then since $sgn(0)$ we are free to choose the value, especially we can set equal to the non thresholded estimators. So it follows $sgn(\hat{\beta}_j^0-\hat{\beta}_{j-1}^0)=sgn(\hat{\beta}_j^{\lambda_1}-\hat{\beta}_{j-1}^{\lambda_1})$. Finally we can plug the soft thresholded estimator in the derivative condition.
	...
	\textbf{Case 2} If then $\hat{\beta}_j^{\lambda_1}=sgn(\hat{\beta}_j^0)(|\hat{\beta}_j^0|-\lambda_1)^{\text{+}}=0$. Ignoring by the same arguments the sign differences we are left with
	\begin{equation}
...
\begin{figure}[h]
	\includegraphics[width=0.6\textwidth]{../../src/sandbox/figures/heatmap_blocks_few_spikes_fused}
