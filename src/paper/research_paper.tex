...
%Structure
The paper is organized as follows. In \cref{chap: las} we introduce lasso, define its optimization problem and investigate some interesting properties. In \cref{chap: fused} we present the fused lasso as an extension of the lasso, describe its behavior and properties. We provide some insights on the solution method. This is followed by a sketch of the fused lasso signal approximator, which is later used for our real data application. Asymptotic results for both lasso and fused lasso are outlined in \cref{chap: asymptotics}. We propose a python implementation of the fused lasso estimator in \cref{implementation}. In \cref{simulation} we present a simulation study comparing the performance of the fused lasso with the lasso and another penalized regression. An application of the fused lasso to genomic data is discussed in \cref{sec: real data}.

\newpage

\section{Lasso} \label{chap: las}
...
For $p \leq N$ and $\text{rank}(X) = p$ there is always a unique solution to \cref{LassoLambda}, since in this case the sum of squared residuals is a strictly convex function of $\beta$.\\
...
\begin{theorem}
	\begin{enumerate}
		\item []
		\item \textbf{Existence} A solution of the minimization problem in \cref{LassoLambda} always exists.
...
The reason for the sparsity property is visualized in \cref{Fused-Signal}.
...
Lasso does not exploit the extra information when estimating. This gives rise to modifications of the lasso. In particular the fused lasso, which is introduced in \cref{chap: fused}.
...
\begin{theorem}(Characterization of the Lasso solution) \label{theo: Kuhntucker} \citep[Chapter 5]{boyd2004convex}
	\begin{align*}
		b_\lambda \in \argmin_{b \in\rm \mathbb{R}^p} S_\lambda(b) \Leftrightarrow 
		\begin{dcases}
...
This often leads to numerous components of the estimators dropping to zero as described in \cref{theo: sparsity_lasso}. \newline

\bigskip

\begin{theorem}(Piecewise linearity)\label{th: convexitylassosolution}
	Let $\hat{\beta}_{\lambda}$ and $\hat{\beta}_\mu$ be solutions of the minimization problem in \cref{LassoLambda} for $\lambda$ respectively $\mu$. Assume the two estimators have the same sign structure, then for any
...
This property can be helpful for calculating solution paths of the lasso. When one has calculated the lasso solution for two different penalty terms, where the estimators have the same sign structure, one does not need to solve the optimization problem for the penalties in between any more, but can simply use the formula for the estimator as in \cref{th: convexitylassosolution}.
...
\end{equation}
where $g:\mathbb{R}^p \to \mathbb{R}$, differentiable and convex and $h_j(\beta_j)$ are convex. Set $g(\beta_1,...,\beta_p)=\sum_{i=1}^{N}(y_{i}-x_{i}'\beta)^2$ and $h_j(\beta_j)=|\beta_j|$ for all j, then the above expression becomes the lasso objective function as in \cref{LassoLambda}. Note that each $h_j(\beta_j)$ function only depends on one coefficient.
...
\begin{equation}\hat\beta_{\lambda,j}^l=
	\begin{dcases} y_j-\frac{\lambda}{2} &if \quad y_j>\frac{\lambda}{2}\\
...
Since both penalty terms are in the objective function, the choice of one penalty constant also influences the optimal choice of the other. Unfortunately the exact impact of that influence is not always intuitive, as can be seen in \cref{simulation}. However one restriction for the choice of the penalty constants can be obtained. Since both penalty terms enter positively in the objective function, the penalty constants should not be as strict, as those of the lasso or of a fused regression.\\

As for the lasso, the penalty constants have to be chosen by the researcher. Choosing the penalty constants incorporates prior information to the estimation. Harsh penalties only makes sense if one believes the true $\beta$ vector to be very sparse or that $\beta_j$ blocks are large. One possible approach for choosing the penalty constants, is cross-validation on a two dimensional grid, which we pursue in section \cref{implementation}.
...
\begin{theorem}
	\begin{enumerate}
		\item[]
		\item \textbf{Existence} A solution of the minimization problem in \cref{eq: fusedlasso} always exists.
...
This makes the model easily interpretable. The piecewise constant solution simply corresponds to an averaging of the features. We will see an example illustrating this property in \cref{sec: real data}. \\
...
$\hat{\beta}^{fl}_j \neq \hat{\beta}^{fl}_{j-1}$ and $t_j \in [-1,1] $ if $\hat{\beta}^{fl}_j = \hat{\beta}^{fl}_{j-1}$.
...
There is a connection between the fusion estimate and the fused lasso, which is described in \cref{sa: soft thresholding}.
...
\section{Asymptotics} \label{chap: asymptotics}
...
\begin{theorem}
	Assume (E) and a structure of $\beta$ as in \cref{eq: beta_fused_signal}. If the conditions on $\lambda_{1,n}$ and $\lambda_{2,n}$, on the minimal size of blocks, the smallest amplitude of blocks and the smallest difference between blocks as spelled out in \citep{rinaldoproperties} are fulfilled then for the set
	\begin{equation}
...
\begin{enumerate}
	...
	\item Since the two penalty constants influence each other (see \cref{subsec: penalties}) they need to be in a certain ratio for each $n$ to enable the detection of the true blocks.
\end{enumerate}
...
Note that from \cref{eq: subder} one can also see that when some of the $\beta_j$s are exactly $0$, the limiting distributions put positive probability at $0$ and when $\beta_1 = \beta_2$ probability is centered at the line $u_1 = u_2$.

\section{Implementation} \label{implementation}

A problem when performing simulations with uncommon estimators, like the fused lasso, is that package availability for that estimator can often be scarce. Also it can not be assumed that the available packages are tested or whether their documentation is correct. After some research we decided to implement our own fused lasso estimator.
...
In \cref{table: mse} we report the results from the simulation. We see that fused lasso and fusion perform well for
...
\begin{table} \label{table: mse}
	\centering
	\scriptsize
	\input{../../out/figures/mytable.tex}
	\caption{Results of the simulation study.}
	\subcaption*{\tiny The values of specificity, sensitivity and blocks are reported as percentages. Standard errors reported brackets.}
\end{table}
\FloatBarrier
...
We have studied two specific penalized regressions for high-dimensional data. The lasso for sparse settings and the fused lasso for sparse block settings. We showed that the fused lasso outperforms lasso in cases for which fused lasso was designed.
...
\lstset{title=Data generating functions}
\pythonexternal{../../../src/data_management/functions_for_data_generation.py}

\lstset{title=Functions which solve the optimization problem}
\pythonexternal{../../../src/model_code/fused_lasso_primal.py}
\pythonexternal{../../../src/model_code/fused_lasso_lagrange.py}

\lstset{title=flestimator.py}
\pythonexternal{../../../src/model_code/flestimator.py}

\lstset{title=Estimation}
\pythonexternal{../../../src/analysis/estimation.py}

\newpage

\lstset{title=Analysis}
\pythonexternal{../../../src/analysis/analysis_of_simulations.py}
...
\begin{proof} The basis of this proof can be found in \citep{friedman2007pathwise}.
	...
	\end{equation}
	where $r_j$ and $t_j$ are as in \cref{theo:subderivatives_fused}.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth,keepaspectratio]{external_figures/lasso_penalty}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth,keepaspectratio]{external_figures/fused_and_lasso_penalty}
...
\begin{figure}[ht]
	\centering
	\begin{subfigure}[l]{.4\textheight}
		\includegraphics[width=1.1\textwidth]{../../out/figures/plot_solutionpath_lasso.png}
		\subcaption{Solution path of the lasso}
	\end{subfigure}
	\begin{subfigure}[l]{.4\textheight}
		\includegraphics[width=1.1\textwidth]{../../out/figures/plot_solutionpath_fused_lasso.png}
		\subcaption{Solution path of the fused lasso for fixed $\lambda_1$ and $log(\lambda_2)$.}
	\end{subfigure}
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Heatmaps}
	\subcaption*{X-axis: s2-values, Y-axis:s1-values}
	\label{heatmap}
\end{figure}
